...
- **Security headers** — cookie-level protections (`httpOnly`, `SameSite`, `Secure` in production) are set by the app; transport and framing headers (`HSTS`, `X-Content-Type-Options`, `X-Frame-Options`/`frame-ancestors`, a CSP suited to the single-route SPA) are enforced at the edge — set them on the hosting proxy/CDN (or a Next.js `middleware.ts`/`next.config.ts` `headers()` block) where TLS terminates.
